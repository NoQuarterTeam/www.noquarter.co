...
<div use:tilt class="relative">
  <MaybeLink href={link} rel="prefetch">
    <div
      class="p-5 md:p-8 flex flex-col justify-between space-y-3 bg-neutral-700/30 shadow-2xl border border-neutral-600/60 rounded-3xl hover:border-neutral-500/60 transition-colors"
    >
      <div class="space-y-3">
  ...
  {#if isLikeable}
    <button
      ...
      class={join(
        "absolute p-4 top-2 right-2 scale-0 delay-200 hover:scale-110 transition-transform opacity-80",
        isLoaded && "scale-100",
      )}
      on:click={toggleSave}
    >
