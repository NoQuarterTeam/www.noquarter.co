...
<div class="space-y-4">
  <div class="flex space-x-2">
    <input
      ...
      class="relative w-full rounded-lg border border-neutral-700 bg-neutral-800/50 py-2 px-6 md:py-3 md:px-8 font-mono shadow-2xl transition-colors placeholder:text-neutral-400 hover:border-neutral-600 hover:bg-white/5 focus:bg-white/5"
  ...
  <div class="scroll flex space-x-2 font-mono w-full overflow-x-auto">
    <button
      on:click={() => toggleTag("About")}
      class={join(
        "rounded-full text-center border border-yellow-600 px-3 py-1 md:px-4 md:py-2 hover:opacity-90 transition-colors",
    ...
    <button
      ...
      class={join(
        "rounded-full text-center border border-teal-700 px-3 py-1 md:px-4 md:py-2 hover:opacity-90 transition-colors",
    ...
    >
    <button class="px-2" on:click={() => filters.update((f) => ({ ...f, showLiked: !f.showLiked }))}>
